...
static void update_double_property(PhysItem *item, GParamSpec *pspec, GtkWidget *label) {
...
static void bind_double_property(GtkSignalListItemFactory *factory, GtkListItem *list_item, gpointer data) {
    (void)factory;
    ...
    g_signal_connect_object(item, signal_name, G_CALLBACK(update_double_property), label, 0);
    g_object_unref(item);
}

static void unbind_double_property(GtkSignalListItemFactory *factory, GtkListItem *list_item, gpointer data) {
    ...
    g_signal_handlers_disconnect_by_func(item, G_CALLBACK(update_double_property), label);
    g_object_unref(item);
}
...
static void bind_check(GtkSignalListItemFactory *factory, GtkListItem *list_item, gpointer data) {
    ...
    g_signal_handlers_unblock_by_func(check, G_CALLBACK(on_check_toggled), NULL);
    g_object_unref(item);
}
...
void set_columns_attribute(GtkApp app)
{
    ...
    g_signal_connect(app->window_main->factories.x, "bind", G_CALLBACK(bind_double_property), "x");
    g_signal_connect(app->window_main->factories.x, "unbind", G_CALLBACK(unbind_double_property), NULL);

    g_signal_connect(app->window_main->factories.y, "setup", G_CALLBACK(setup_label), NULL);
    g_signal_connect(app->window_main->factories.y, "bind", G_CALLBACK(bind_double_property), "y");
    g_signal_connect(app->window_main->factories.y, "unbind", G_CALLBACK(unbind_double_property), NULL);

    g_signal_connect(app->window_main->factories.vx, "setup", G_CALLBACK(setup_label), NULL);
    g_signal_connect(app->window_main->factories.vx, "bind", G_CALLBACK(bind_double_property), "vx");
    g_signal_connect(app->window_main->factories.vx, "unbind", G_CALLBACK(unbind_double_property), NULL);

    g_signal_connect(app->window_main->factories.vy, "setup", G_CALLBACK(setup_label), NULL);
    g_signal_connect(app->window_main->factories.vy, "bind", G_CALLBACK(bind_double_property), "vy");
    g_signal_connect(app->window_main->factories.vy, "unbind", G_CALLBACK(unbind_double_property), NULL);

    g_signal_connect(app->window_main->factories.ax, "setup", G_CALLBACK(setup_label), NULL);
    g_signal_connect(app->window_main->factories.ax, "bind", G_CALLBACK(bind_double_property), "ax");
    g_signal_connect(app->window_main->factories.ax, "unbind", G_CALLBACK(unbind_double_property), NULL);

    g_signal_connect(app->window_main->factories.ay, "setup", G_CALLBACK(setup_label), NULL);
    g_signal_connect(app->window_main->factories.ay, "bind", G_CALLBACK(bind_double_property), "ay");
    g_signal_connect(app->window_main->factories.ay, "unbind", G_CALLBACK(unbind_double_property), NULL);

    g_signal_connect(app->window_main->factories.mass, "setup", G_CALLBACK(setup_label), NULL);
    g_signal_connect(app->window_main->factories.mass, "bind", G_CALLBACK(bind_double_property), "mass");
    g_signal_connect(app->window_main->factories.mass, "unbind", G_CALLBACK(unbind_double_property), NULL);

    g_signal_connect(app->window_main->factories.check, "setup", G_CALLBACK(setup_check), app);
