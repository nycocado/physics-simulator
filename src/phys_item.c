...
static void phys_item_set_property(
    GObject* object,
    guint property_id,
    const GValue* value,
    GParamSpec* pspec
...
static void phys_item_get_property(
    GObject* object,
    guint property_id,
    GValue* value,
    GParamSpec* pspec
...
{
    return g_object_new(
        PHYS_TYPE_ITEM, "item-type", PHYS_ITEM_FORCE, "ax", ax, "ay", ay, NULL
    );
}
...
void phys_item_remove_child(PhysItem* item, PhysItem* child)
{
    ...
    {
        g_autoptr(PhysItem) current =
            g_list_model_get_item(G_LIST_MODEL(item->children), i);
        if (current == child)
        {
