...
void test_position(void)
{
    ...
    TEST_ASSERT_DOUBLE_WITHIN(PRECISION, 15, phyc_position(3, 0.5, 10, 1.5));
}
...
void test_velocity(void)
{
    ...
    TEST_ASSERT_DOUBLE_WITHIN(
        PRECISION, 18.745, phyc_velocity(5.6, 2.32, 5.666)
    );
...
void test_medium_velocity(void)
{
    ...
    TEST_ASSERT_DOUBLE_WITHIN(
        PRECISION, 2.5, phyc_medium_velocity(10, 5, 2.5, 0.5)
    );
    TEST_ASSERT_DOUBLE_WITHIN(
        PRECISION, 0.726, phyc_medium_velocity(13, 7, 9, 0.737)
    );
}
...
void test_medium_acceleration(void)
{
    ...
    TEST_ASSERT_DOUBLE_WITHIN(
        PRECISION, 4.666, phyc_medium_acceleration(1, 15, 3)
    );
    TEST_ASSERT_DOUBLE_WITHIN(
        PRECISION, 0.415, phyc_medium_acceleration(0, 32, 77)
    );
    TEST_ASSERT_DOUBLE_WITHIN(
        PRECISION, 0.4396, phyc_medium_acceleration(2.5, 7.776, 12)
    );
    TEST_ASSERT_DOUBLE_WITHIN(
        PRECISION, 16.915, phyc_medium_acceleration(0, 80.35, 4.75)
    );
}
...
void test_degree_to_radian(void)
{
    ...
    TEST_ASSERT_DOUBLE_WITHIN(PRECISION, M_PI / 6, phyc_degree_to_radian(30));
    TEST_ASSERT_DOUBLE_WITHIN(PRECISION, M_PI / 4, phyc_degree_to_radian(45));
    TEST_ASSERT_DOUBLE_WITHIN(PRECISION, M_PI / 3, phyc_degree_to_radian(60));
    TEST_ASSERT_DOUBLE_WITHIN(PRECISION, M_PI / 2, phyc_degree_to_radian(90));
    TEST_ASSERT_DOUBLE_WITHIN(PRECISION, M_PI, phyc_degree_to_radian(180));
    TEST_ASSERT_DOUBLE_WITHIN(
        PRECISION, 3 * M_PI / 2, phyc_degree_to_radian(270)
    );
    TEST_ASSERT_DOUBLE_WITHIN(PRECISION, 2 * M_PI, phyc_degree_to_radian(360));
}

void test_radian_to_degree(void)
{
    TEST_ASSERT_DOUBLE_WITHIN(PRECISION, 0, phyc_radian_to_degree(0));
    TEST_ASSERT_DOUBLE_WITHIN(PRECISION, 30, phyc_radian_to_degree(M_PI / 6));
    TEST_ASSERT_DOUBLE_WITHIN(PRECISION, 45, phyc_radian_to_degree(M_PI / 4));
    TEST_ASSERT_DOUBLE_WITHIN(PRECISION, 60, phyc_radian_to_degree(M_PI / 3));
    TEST_ASSERT_DOUBLE_WITHIN(PRECISION, 90, phyc_radian_to_degree(M_PI / 2));
    TEST_ASSERT_DOUBLE_WITHIN(PRECISION, 180, phyc_radian_to_degree(M_PI));
    TEST_ASSERT_DOUBLE_WITHIN(
        PRECISION, 270, phyc_radian_to_degree(3 * M_PI / 2)
    );
